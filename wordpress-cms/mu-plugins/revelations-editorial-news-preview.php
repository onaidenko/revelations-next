<?php
/**
 * Plugin Name: REVELATIONS Editorial News Preview
 * Description: Admin-only News RSS preview control.
 * Version: 0.1.0
 */
...
/**
 * Handle the News preview request.
 *
 * The shared runner blocks the request while News is disabled.
 */
add_action(
    'admin_post_revelations_preview_news_scan',
...
/**
 * Render the News preview card.
 */
function revelations_editorial_render_news_preview(): void {
    if ( ! current_user_can( 'manage_options' ) ) {
        return;
    }
    ...
    $candidate_error = 'news' === $candidate_section
        ? sanitize_key(
            wp_unslash(
    ...
    if (
        ...
        <div class="notice notice-success is-dismissible">
            <p>
                News scan completed in dry-run mode.
    ...
    if (
        ...
        <div class="notice notice-error is-dismissible">
            <p>
                <?php echo esc_html(
                    'disabled' === $scan_error
                        ? 'The News scanner is disabled in Settings.'
                        : 'The News scanner is not available.'
                ); ?>
            </p>
        </div>
        <?php
    ...
    >
        <div class="revelations-tech-preview__header">
            <div>
                ...
                <p class="revelations-desk__section-description">
                    Preview qualified stories from active News
            ...
            >
                <input
                    type="hidden"
                    name="action"
                    value="revelations_preview_news_scan"
                >
        ...
        <?php if ( ! is_array( $preview ) ) : ?>
            <p>
                No News preview has been generated in this session.
            </p>
        <?php else : ?>
            <?php
            $qualified = isset(
            ...
            <?php if ( array() === $qualified ) : ?>
                <p>
                    No stories passed the current News editorial rules.
                </p>
            <?php else : ?>
