...
function revelations_editorial_ai_research_web_urls( array $response ): array {
    $urls = array(); $walk = static function ( mixed $value ) use ( &$walk, &$urls ): void { if ( ! is_array( $value ) ) return; if ( isset( $value['url'] ) && is_string( $value['url'] ) && preg_match( '#^https://#i', $value['url'] ) ) $urls[ esc_url_raw( $value['url'] ) ] = true; foreach ( $value as $child ) $walk( $child ); };
    foreach ( (array) ( $response['output'] ?? array() ) as $item ) if ( is_array( $item ) && 'web_search_call' === ( $item['type'] ?? '' ) ) $walk( $item );
    return $urls;
}
...
function revelations_editorial_ai_research_evidence_pack( array $config, string $section, array $lead, string $headline, string $summary, string $snapshot ) {
    ...
    $body = array( 'model' => $config['model'], 'instructions' => $instructions, 'input' => $input, 'tools' => array( array( 'type' => 'web_search' ) ), 'reasoning' => array( 'effort' => 'none' ), 'text' => array( 'format' => array( 'type' => 'json_schema', 'name' => 'revelations_factual_research', 'strict' => true, 'schema' => revelations_editorial_ai_research_schema() ) ), 'max_output_tokens' => 3500, 'store' => false );
    ...
    if ( is_wp_error( $response ) ) return new WP_Error( 'research_failed', 'Independent research request failed.' );
    $status = (int) wp_remote_retrieve_response_code( $response ); $decoded = json_decode( wp_remote_retrieve_body( $response ), true );
    if ( ! is_array( $decoded ) || $status < 200 || $status >= 300 || 'completed' !== ( $decoded['status'] ?? '' ) ) return new WP_Error( 'research_failed', 'Independent research could not be completed.' );
    $research = json_decode( revelations_editorial_ai_generation_extract_text( $decoded ), true ); if ( ! is_array( $research ) || ! is_array( $research['sources'] ?? null ) ) return new WP_Error( 'research_failed', 'Independent research returned no usable evidence.' );
    $web_urls = revelations_editorial_ai_research_web_urls( $decoded ); if ( array() === $web_urls ) return new WP_Error( 'research_unverifiable', 'Independent research did not return verifiable web-search sources.' );
    ...
    foreach ( $research['sources'] as $source ) {
        ...
        if ( '' === $url || '' === $host || ! isset( $web_urls[ $url ] ) || isset( $sources[ $url ] ) || $host === ( $lead['host'] ?? '' ) ) continue;
        ...
        $sources[ $url ] = array( 'url' => $url, 'name' => sanitize_text_field( (string) ( $source['name'] ?? $host ) ), 'publication_date' => sanitize_text_field( (string) ( $source['publication_date'] ?? '' ) ), 'source_type' => $type, 'reliability' => $reliability, 'host' => $host, 'claims' => $claims ); $hosts[ $host ] = true; if ( 'primary_authoritative' === $reliability ) ++$primary_count;
    }
    if ( count( $sources ) < 2 || count( $hosts ) < 2 ) return new WP_Error( 'insufficient_independent_evidence', 'Independent research requires two distinct corroborating sources beyond the lead.' );
    if ( ! empty( $lead['requires_independent_corroboration'] ) && $primary_count < 1 ) return new WP_Error( 'insufficient_independent_evidence', 'This lead type requires corroborating primary authoritative evidence.' );
