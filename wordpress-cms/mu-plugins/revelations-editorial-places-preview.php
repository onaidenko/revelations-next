<?php
/**
 * Plugin Name: REVELATIONS Editorial Places Preview
 * Description: Admin-only Places RSS preview control.
 * Version: 0.1.0
 */
...
/**
 * Handle the Places preview request.
 *
 * The shared runner blocks the request while Places is disabled.
 */
add_action(
    'admin_post_revelations_preview_places_scan',
...
/**
 * Render the Places preview card.
 */
function revelations_editorial_render_places_preview(): void {
    if ( ! current_user_can( 'manage_options' ) ) {
        return;
    }
    ...
    $candidate_error = 'places' === $candidate_section
        ? sanitize_key(
            wp_unslash(
    ...
    if (
        ...
        <div class="notice notice-success is-dismissible">
            <p>
                Places scan completed in dry-run mode.
    ...
    $scan_error = 'places' === $scan_section
        ? sanitize_key(
            wp_unslash(
                $_GET['scan_error'] ?? ''
            )
    ...
    if (
        ...
        <div class="notice notice-error is-dismissible">
            <p>
                <?php echo esc_html(
                    'disabled' === $scan_error
                        ? 'The Places scanner is disabled in Settings.'
                        : 'The Places scanner is not available.'
                ); ?>
            </p>
        </div>
        <?php
    ...
    >
        <div class="revelations-tech-preview__header">
            <div>
                <h2>Places RSS scan</h2>

                <p class="revelations-desk__section-description">
                    Preview qualified stories from active Places
            ...
            >
                <input
                    type="hidden"
                    name="action"
                    value="revelations_preview_places_scan"
                >
        ...
        <?php if ( ! is_array( $preview ) ) : ?>
            <p>
                No Places preview has been generated in this session.
            </p>
        <?php else : ?>
            <?php
            $qualified = isset(
            ...
            <?php if ( array() === $qualified ) : ?>
                <p>
                    No stories passed the current Places editorial rules.
                </p>
            <?php else : ?>
