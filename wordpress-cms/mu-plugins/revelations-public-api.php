    ...
    if ( function_exists( 'revelations_author_relation' ) ) {
        foreach ( revelations_author_relation( get_post_meta( $post_id, '_revelations_author_profile_ids', true ) ) as $author_id ) {
            ...
            if ( $author instanceof WP_Post && function_exists( 'revelations_author_public_data' ) ) {
                $count = new WP_Query( array( 'post_type'=>'post', 'post_status'=>'publish', 'meta_key'=>'_revelations_author_profile_ids', 'meta_value'=>(string) $author_id, 'meta_compare'=>'LIKE', 'posts_per_page'=>1, 'fields'=>'ids' ) );
...
function revelations_api_authors( WP_REST_Request $request ): WP_REST_Response {
    ...
    foreach ( $authors as $author ) { $count=(new WP_Query(array('post_type'=>'post','post_status'=>'publish','meta_key'=>'_revelations_author_profile_ids','meta_value'=>(string) $author->ID,'meta_compare'=>'LIKE','posts_per_page'=>1,'fields'=>'ids')))->found_posts; $data=function_exists('revelations_author_public_data')?revelations_author_public_data($author,(int)$count):null; if($data)$items[]=$data; }
...
function revelations_api_author( WP_REST_Request $request ) {
    ...
    $articles = get_posts( array( 'post_type'=>'post','post_status'=>'publish','meta_key'=>'_revelations_author_profile_ids','meta_value'=>(string) $author->ID,'meta_compare'=>'LIKE','posts_per_page'=>100,'orderby'=>'date','order'=>'DESC' ) );
