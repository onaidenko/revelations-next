...
function revelations_taxonomy_import_apply( array $map ): int {
    ...
    foreach ( $map['assignments'] as $a ) { $id=$runtime['resolved'][$a['slug']]['id'] ?? 0; if(!$id){throw new RuntimeException('unresolved_post');} $topic_ids=array_map(static fn($s)=>$terms['revelations_topic'][$s],array_merge(array($a['primary_topic']),$a['secondary_topics'])); wp_set_object_terms($id,$topic_ids,'revelations_topic',false); wp_set_object_terms($id,$a['series'] ? array($terms['revelations_series'][$a['series']]) : array(),'revelations_series',false); wp_set_object_terms($id,$a['location'] ? array($terms['revelations_location'][$a['location']]) : array(),'revelations_location',false); update_post_meta($id,'_revelations_primary_topic',$terms['revelations_topic'][$a['primary_topic']]); update_post_meta($id,'_revelations_public_topic_eligible',$a['public_topic_eligible']?'1':'0'); update_post_meta($id,'_revelations_taxonomy_status',$a['taxonomy_status']); $writes+=6; }
    foreach ( $map['manual_related'] as $o ) { $ids=array_map(static fn($s)=>$runtime['resolved'][$s]['id'], $o['target_slugs']); update_post_meta($runtime['resolved'][$o['source_slug']]['id'],'_revelations_manual_related',$ids); $writes++; }
    return $writes;
}
