...
- `origin/admin-editorial` остаётся на `afc5dfd`; локальные коммиты ещё не отправлены.
...
Следующий функциональный шаг — реальный scanner dry-run, но только
после отдельного разрешения. Push и deploy также не выполнять без
