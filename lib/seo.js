...
export function buildPageMetadata({
  ...
  const socialImage = toAbsoluteUrl(fallbackImage, siteUrl);
  ...
  return {
    ...
    twitter: {
      ...
      description,
      images: socialImage
        ? [{ url: socialImage, alt: 'REVELATIONS' }]
        : undefined,
    },
  };
}

export function buildSectionBreadcrumbJsonLd(section, siteUrl) {
  const canonical = toAbsoluteUrl(`/${section?.id}`, siteUrl);

  if (!section?.id || !section?.title || !canonical) {
    return null;
  }
...
export function buildAuthorBreadcrumbJsonLd(author, siteUrl) {
  const canonical = toAbsoluteUrl(
    `/authors/${author?.slug}`,
    siteUrl
  );

  if (!author?.slug || !author?.name || !canonical) {
    return null;
  }
  ...
  return {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'People',
        item: toAbsoluteUrl('/people', siteUrl),
      },
      {
        '@type': 'ListItem',
        position: 3,
...
export function buildMainSitemapEntries({
  ...
  [...taxonomyHubs]
    .filter((hub) => hub?.pathname && hub?.articles?.length)
    .sort((first, second) =>
    ...
    .forEach((hub) => {
      ...
      add({
        url: toAbsoluteUrl(hub.pathname, siteUrl),
        ...(timestamps.length
          ? {
...
export function buildArticleMetadata(
  ...
  return {
    ...
    openGraph: {
      ...
      authors: article.author_profiles?.length
        ? article.author_profiles.map((profile) => profile.name).filter(Boolean)
        : article.author ? [article.author] : undefined,
...
export function buildOrganizationJsonLd({
  ...
  return {
    ...
    '@id': organizationId,
    name: siteName,
    alternateName: alternateNames.length
      ? alternateNames
      : undefined,
    ...
    location: locationName
      ...
      : undefined,
    parentOrganization: publisherBrandName
      ? {
          '@type': 'Organization',
          name: publisherBrandName,
        }
      : undefined,
  };
}
...
export function buildArticleJsonLd(
  ...
  const canonical = articleCanonicalUrl(article, siteUrl);
  const coverImage = toAbsoluteUrl(article.cover_image, siteUrl);
  const author = article.author_profiles?.length
    ? article.author_profiles.map((profile) => profile.is_public_profile && profile.url
      ? { '@id': `${profile.url}#${profile.schema_type === 'organization' ? 'organization' : 'person'}` }
      : { '@type': profile.schema_type === 'organization' ? 'Organization' : 'Person', name: profile.name })
    : article.author
...
export function buildPodcastSeriesJsonLd({
  ...
  const canonical = toAbsoluteUrl('/podcast', siteUrl);

  if (!canonical || !name || !description || !organizationId) {
    return null;
  }
...
export function buildPodcastEpisodeJsonLd(
  ...
  const image = toAbsoluteUrl(article?.cover_image, siteUrl);
  const videoUrl = toAbsoluteUrl(article?.youtube_url, siteUrl);
...
export function buildTaxonomyMetadata(
  hub,
  { siteUrl, siteName = 'REVELATIONS' }
) {
  const canonical = toAbsoluteUrl(
    hub.pathname,
    siteUrl
  );
  const count = hub.articles.length;
  const title = hub.seoTitle || hub.name;
  const description =
    hub.seoDescription ||
    `Explore ${count} ${
      count === 1 ? 'story' : 'stories'
    } connected to ${hub.name} on ${siteName}.`;
...
export function buildTaxonomyBreadcrumbJsonLd(
  hub,
  siteUrl
) {
  const canonical = toAbsoluteUrl(
...
export function buildTaxonomyCollectionJsonLd(
  hub,
  siteUrl
) {
  const canonical = toAbsoluteUrl(
  ...
  return {
    ...
    mainEntity: {
      '@type': 'ItemList',
      numberOfItems: hub.articles.length,
      itemListElement: hub.articles.map(
        (article, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: article.title,
...
export function buildTaxonomyIndexBreadcrumbJsonLd({
  name,
  pathname,
  siteUrl,
}) {
...
export function buildTaxonomyIndexCollectionJsonLd({
  name,
  description,
  pathname,
  hubs,
  ...
  const canonical = toAbsoluteUrl(pathname, siteUrl);
  const items = (Array.isArray(hubs) ? hubs : []).filter(
    (hub) => hub?.name && hub?.pathname
  );
  ...
  return {
    ...
    mainEntity: {
      ...
      itemListElement: items.map((hub, index) => ({
        '@type': 'ListItem',
        position: index + 1,
        name: hub.name,
        url: toAbsoluteUrl(hub.pathname, siteUrl),
