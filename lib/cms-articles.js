...
function normalizeCmsArticle(article) {
  ...
  return {
    ...
    tags,
    primary_topic: article.primary_topic?.slug || '',
    topics: Array.isArray(article.topics) ? article.topics.map((topic) => topic?.slug).filter(Boolean) : [],
    series: article.series?.slug || '',
    locations: Array.isArray(article.locations) ? article.locations.map((location) => location?.slug).filter(Boolean) : [],
    public_topic_eligible: article.public_topic_eligible !== false,
    taxonomy_status: article.taxonomy_status || 'proposed',
    manual_related: Array.isArray(article.manual_related) ? article.manual_related.map((item) => item?.slug).filter(Boolean) : [],
...
export async function getRelatedArticles(
  ...
  return selectRelatedArticles(article, articles, limit);
}
